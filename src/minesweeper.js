...
const getNumberOfNeighborBombs = (bombBoard, rowIndex, columnIndex) => {
  ...
  neighborOffsets.forEach(offset => {
    ...
    const neighborColumnIndex = rowIndex+ offset[1];
...
const flipTile = (playerBoard,bombBoard,rowIndex,columnIndex) => {
  if (playerBoard[rowIndex][columnIndex] !== ' ') {
    console.log('This tile has alredy been flipped!');
    return;
  }else if (bombBoard[neighborRowIndex][neighborColumnIndex] === 'B') {
    bombBoard[neighborRowIndex][neighborColumnIndex];
  }else {
...
let bombBoard = generateBombBoard(5,3,5);
